...
pub type ScoreFn = Box<dyn Fn(&ArrayRef, &BooleanArray) -> Option<f64>>;
...
fn logit(target: ArrayRef, prediction: f64) -> f64 {
...
pub fn generate_score_function(score_config: &ScoreConfig) -> ScoreFn {
    let (score_fn, is_wgt): (Box<dyn Fn(ArrayRef) -> f64>, bool) = match score_config.score_function
    {
        ScoreFunction::Weighted(weighted_fn) => {
            ...
            (
                match weighted_fn {
                    WeightedScoreFunction::Gini => Box::new(|arr| gini(arr)),
                },
                true,
            )
        }
        ScoreFunction::Differentiable(diff_fn) => {
            ...
            (
                match diff_fn {
                    DifferentiableScoreFunction::Logit => Box::new(move |arr| logit(arr, pred)),
                },
                false,
            )
        }
    };
    Box::new(move |arr: &ArrayRef, filter_mask: &BooleanArray| {
        let arr = arr.clone();
